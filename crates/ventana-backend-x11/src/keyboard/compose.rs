...
pub struct XkbComposeTable {
  table: ThreadPtr<xkb_compose_table>,
}
...
impl XkbComposeTable {
  ...
  pub fn new_state(&self) -> Option<XkbComposeState> {
    let state = unsafe {
      (XKBCH.xkb_compose_state_new)(self.table.as_ptr(), xkb_compose_state_flags::XKB_COMPOSE_STATE_NO_FLAGS)
    };
...
impl Deref for XkbComposeTable {
  type Target = ThreadPtr<xkb_compose_table>;
...
pub struct XkbComposeState {
  state: ThreadPtr<xkb_compose_state>,
}
